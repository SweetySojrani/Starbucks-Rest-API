...
func initRoutes(mx *mux.Router, formatter *render.Render) {
	...
	mx.HandleFunc("/order/{id}", getOrderStatusHandler(formatter)).Methods("GET")
	mx.HandleFunc("/order/{id}", completeOrderHandler(formatter)).Methods("POST")
	mx.HandleFunc("/orders", getHistoryOrderHandler(formatter)).Methods("GET")
}
...
func createOrderHandler(formatter *render.Render) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		query := bson.M{"UserId": m.UserId, "OrderId": m.OrderId, "Items": m.Items, "OrderStatus": m.OrderStatus}
		...
		var result bson.M
		err = c.Find(bson.M{"OrderId": m.OrderId, "UserId": m.UserId}).One(&result)
		if err != nil {
			log.Fatal(err)
		}
